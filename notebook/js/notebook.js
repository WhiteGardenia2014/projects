window.addEventListener('DOMContentLoaded', function () {
  ...
  // Todo 相关
  let toggleTodoButton = this.document.querySelector('.toggleTodo') // 切换 todo 显示面板按钮
  let todoapp = this.document.querySelector('.todoapp') // todo 面板
  ...
  toggleTodoButton.addEventListener('click', function () {
    if (todoapp.style.opacity == 0) {
      todoapp.style.opacity = 1
      todoapp.style.visibility = 'visible'
    } else {
      todoapp.style.opacity = 0
      todoapp.style.visibility = 'hidden'
    }
  })
})
